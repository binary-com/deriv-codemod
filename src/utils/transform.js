module.exports = function (file, api, options) {
    ...
    const isReactPropTypes = (path) => {
        if (path.type === 'AssignmentExpression') {
            return (
                !!path.left?.object?.name && path.left?.property?.name === 'propTypes'
            );
        }
        return false;
    };
            ...
            .forEach((path) => {
              ...
              ['observer'].includes(init.callee.name)
                            ...
                            if (init.arguments[0]?.params?.length > 0) {
                                init.arguments[0].params[0].typeAnnotation =
                  j.tsTypeAnnotation(typeRef);
    ...
    function insertTsTypes(componentPropTypesMap) {
        ...
        Object.keys(componentPropTypesMap).forEach((componentName) => {
            ...
            Object.keys(properties).forEach((key) => {
                ...
                const propertySignature = j.tsPropertySignature(
                    j.identifier(key),
                    j.tsTypeAnnotation(tsType),
                );
            ...
            const type = j.tsTypeAliasDeclaration(
    ...
    // Skip isRequired and finds the path of the type
        ...
        if (key === 'children') {
            return j.tsTypeReference(
                j.tsQualifiedName(j.identifier('React'), j.identifier('ReactNode')),
            );
        }
            ...
            .forEach((path) => {
                if (path.value.right.properties.length >= 0) {
                    ...
                    path.value.right.properties.forEach((property) => {
                        if (property.key) {
                            componentPropTypesMap[fnComponentName][property.key.name] =
                getTSType(property.key.name, property.value);
                        }
                    });
                }
            });
